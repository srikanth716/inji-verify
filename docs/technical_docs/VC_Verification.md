...
**`isVPSubmissionSupported=false`:** SDK redirects directly to the Online VC Provider with `client_id` and `redirect_uri`. The provider returns an OpenID4VP 1.0 DCQL `vp_token` in the redirect hash (URL-encoded or plain JSON keyed by credential query id). The SDK extracts the VC and submits it to `/v2/vc-verification`.
